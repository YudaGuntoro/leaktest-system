...
public static class LeakTestWorkRecordListReportBuilder
{
        ...
        string templatePath)
    {
        ...
        var logoPath = Path.Combine(templateDirectory, LeakTestWorkRecordReportBuilder.LogoFileName);
        ...
        worksheet.PageSetup.PrintAreas.Add($"B2:K{lastRow}");
    ...
    private static void ApplyLayout(IXLWorksheet worksheet)
    {
        ...
        worksheet.Column("J").Width = 17;
        worksheet.Column("K").Width = 11;
        worksheet.Column("L").Width = 2;
        ...
        worksheet.Range("B2:K4").Style.Border.OutsideBorderColor = XLColor.FromHtml("#0F172A");
        ...
        var tableHeader = worksheet.Range("B12:K12");
    ...
    private static int FillTable(IXLWorksheet worksheet, IReadOnlyList<LeakTestWorkRecord> records)
    {
        ...
        if (records.Count == 0)
        {
            var emptyRange = worksheet.Range("B13:K15");
            emptyRange.Merge();
        ...
        for (var index = 0; index < records.Count; index++)
        {
            ...
            worksheet.Cell(row, 5).Value = FormatDate(record.CheckDate);
            worksheet.Cell(row, 6).Value = FormatTime(record.CheckTime);
            worksheet.Cell(row, 7).Value = record.MachineName;
            ...
            worksheet.Cell(row, 10).Value = FormatMinutes(record.CycleTimeLeakTestMinutes);
            worksheet.Cell(row, 11).Value = record.Result;

            var rowRange = worksheet.Range(row, 2, row, 11);
            ...
            worksheet.Range(row, 5, row, 6).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;
            worksheet.Range(row, 8, row, 11).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;
            worksheet.Range(row, 3, row, 4).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Left;
        ...
        worksheet.Range(HeaderRow, 2, lastRow, 11).SetAutoFilter();
    ...
    private static bool TryAddYanmarLogo(IXLWorksheet worksheet, string logoPath)
    {
        ...
        worksheet.AddPicture(logoStream, XLPictureFormat.Png, "Yanmar Mark")
            .MoveTo(worksheet.Cell("B2"), 41, 25)
            .WithSize(80, 40);
        return true;
    }
